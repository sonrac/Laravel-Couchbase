...
```php
$app->register(sonrac\Couchbase\CouchbaseServiceProvider::class);
...
{
    return new sonrac\Couchbase\Connection($config);
});
```
...
```php
'CouchbaseModel'       => 'sonrac\Couchbase\Eloquent\Model',
```
...
If you're using a hybrid Couchbase and SQL setup, you're in luck! The model will automatically return a Couchbase- or SQL-relation based on the type of the related model. Of course, if you want this functionality to work both ways, your SQL-models will need use the `sonrac\Couchbase\Eloquent\HybridRelations` trait. Note that this functionality only works for hasOne, hasMany and belongsTo relations.

Example SQL-based User model:

```php
use sonrac\Couchbase\Eloquent\HybridRelations;
